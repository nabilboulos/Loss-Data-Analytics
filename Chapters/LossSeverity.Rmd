...
$$f_{X}\left( x \right) = \frac{\left( x / \theta \right)^{\alpha}}{x\Gamma\left( \alpha \right)} e^{- x / \theta} $$ for $x > 0$. For $\alpha>0$, $\color{red}{\text{the k-th raw moment is}}$
<!-- actually:*k*-th -->
...
Given $\Gamma\left( r + 1 \right) = r\Gamma\left( r \right)$ and $\color{red}{\Gamma\left( 1 \right) = 1}$, then $\mu_{1}^{\prime} = E\left( X \right) = \alpha\theta$, $\mu_{2}^{\prime} = E\left( X^{2} \right) = \left( \alpha + 1 \right)\alpha\theta^{2}$, $\mu_{3}^{\prime} = E\left( X^{3} \right) = \left( \alpha + 2 \right)\left( \alpha + 1 \right)\alpha\theta^{3}$, and 
$\mathrm{Var}\left( X \right) = \color{red}{(\alpha  + 1)\alpha\theta^2 - (\alpha\theta)^2} = \alpha\theta^{2}$.
...
Hence, $\alpha = 4$. Since, $E\left( X \right) = \alpha\theta = 8$, then $\theta = 2$ and $\color{red}{finally,} \mathrm{Var}\left( X \right) = \alpha\theta^{2} = 16$.
...
$F_{X}\left( x \right) = 1 - e^{- \theta x}$. So, $F_{X}\left( \pi_{0.5} \right) = 1 - e^{- \theta\pi_{0.5}} = 0.5$. $\color{red}{As, \pi_{0.5} = \frac{1}{3},}$ we have $\color{red}{F_X\left(\frac{1}{3}\right) = } 1 - e^{-\theta / 3} = 0.5$ and $\theta = 3 \ln 2$.
</div>
...
$\color{red}{\text{As},\ X\sim Exp \left( \frac{1}{b}\right),}$
...
Let $X_{1}$, $X_{2}$, ., $X_{n}$ be independent $\text{Ga}\left( \alpha_{i},\theta \right)$ random variables. Find the distribution of $S = \sum_{i = 1}^{n}X_{i}$.
...
= E\left( \prod_{i = 1}^{n}e^{tX_{i}} \right)$$ $\color{red}{\text{using independence we get}}$ $$= \prod_{i = 1}^{n}{E\left( e^{tX_{i}} \right) = \prod_{i = 1}^{n}{M_{X_{i}}\left( t \right)}} .$$
...
We can also use the probability generating function to generate moments of $X$. By taking the *k*-th derivative of $P_{X}\left( z \right)$ with respect to $z$ and evaluate it at $z\  = \ 1$, we get $\mathrm{E}\left\lbrack X\left( X - 1 \right)\ldots\left( X - k + 1 \right) \right\rbrack .$
...
```{r Paretopdf, fig.cap='Pareto Densities. The left-hand panel is with scale=2000 and Varying Shape.  The right-hand panel is with shape=3 and Varying Scale', out.width='120%', fig.asp=.75, fig.align='center', echo=FALSE}
library(VGAM)
...
$\color{red}{1.\text{ As},\ X\sim Pa(\alpha,\theta),\text{ we have }}\mathrm{E}\left( X \right) = \frac{\theta}{\alpha - 1} = 40$ and
...
$\color{red}{2.\ }$The 95-th percentile, $\pi_{0.95}$, satisfies the equation
...
$\color{red}{1.\ }$Let $X \color{red}{\sim Wei \left( 1.2,33.33 \right)}$ be the lifetime of AIDS patients (in months). We have,
$${\Pr\left( X \geq 12 \right) = S}_{X}\left( 12 \right) = e^{- \left( \frac{12}{33.33} \right)^{1.2}} = 0.746.$$
$\color{red}{2.\ }$Let $Y$ be the number of patients who die within one year of diagnosis. Then, $Y\sim Bin\left( 10,\ 0.254 \right)$ and $\Pr\left( Y \leq 2 \right) = 0.514.$

$\color{red}{3.\ }$Let $\pi_{0.99}$ denote the 99-th percentile of this distribution. Then,
...
Let $X$ and $Y$ denote the aggregate losses of Eiffel Auto Insurance in euro currency and dollars respectively. $\color{red}{\text{As, }}$ $Y = 1.3X$, $\color{red}{\text{we have,}}$
...
$\color{red}{As \left| \frac{dx}{dy} \right| = \frac{1}{1.3}}$, the probability density function of interest $f_{Y}(y)$ is
...
Let $X\sim Ga(\alpha,\theta)$ and $Y = cX$. $\color{red}{As \left| \frac{dx}{dy} \right| = \frac{1}{c}},$ then
...
$\color{red}{\text{As},\ X\sim Exp(\theta),\text{ so}}$
$$f_{X}(x) = \frac{1}{\theta}e^{- x/ \theta} \ \ \ \, x > 0.$$
$\color{red}{\text{Solving for X and taking the derivative, we have} \left| \frac{dx}{dy} \right| = \frac{1}{\tau}{y^{\frac{1}{\tau} - 1}},\text{ so}}$
...
$\color{red}{\text{We begin with the c.d.f of } Y,}$
$$F_{Y}\left( y \right) = \Pr\left( Y \leq y \right) = \Pr\left( e^{X} \leq y \right) = \Pr\left( X \leq \ln y \right) = F_{X}\left( \ln y \right).$$
$\color{red}{\text{Taking the derivative, we have,}}$
...
The distribution of the random variable $X$ is an equally weighted mixture of two Poisson distributions with parameters $\lambda_{1}$ and $\lambda_{2}$ respectively. The mean and variance of $X$ are 4 and 13, respectively. Determine $\Pr\left( X > 2 \right)$.
...
$$\mathrm{E}\left( X \right) = 0.5\lambda_{1} + 0.5\lambda_{2} = 4$$

$$\mathrm{E}\left( X^{2} \right) = \color{red}{V(X) + \left[ E(X) \right]^2 = } 0.5\left( \lambda_{1} + \lambda_{1}^{2} \right) + 0.5\left( \lambda_{2} + \lambda_{2}^{2} \right) = 13 + 16$$

$\color{red}{\text{Substituting the first in the second equation}}$ we get $\lambda_{1} + \lambda_{2} = 8$ and $\lambda_{1}^{2} + \lambda_{2}^{2} = 50$. $\color{red}{\text{After substituting further we get that}}$ the parameters of the two Poisson distributions are 1 and 7 $\color{red}{\text{, respectively. So,}}$
...
**Solution.**
$\color{red}{\text{Applying the formula for a mixed distribution, we get that}}$
...
$\color{red}{\text{Substituting the first two equations in the third, we get}}$
...
Then, $Z$ is a mixture of $X_{1}$, $X_{2}$, $X_{3}$ and $X_{4}$, with mixing weights $\text{ca}$, $c\left( 1 - a \right)$, $\left( 1 - c \right)b$ and $(1 - c)\left( 1 - b \right),\color{red}{\text{ respectively}}$.
...
$X$ has a binomial distribution with a mean of $100q$ and a variance of $100q\left( 1 - q \right)$ and $q$ has a beta distribution with parameters $a = 3$ and $b = 2$. Find the unconditional mean and variance of $X$.
...
$\color{red}{\text{As }q\sim Beta(3,2),\text{ we have}}$
$\mathrm{E}\left( q \right) = \frac{a}{a + b} = \frac{3}{5}$ and
$\mathrm{E}\left( q^{2} \right) = \frac{a\left( a + 1 \right)}{\left( a + b \right)\left( a + b + 1 \right)} = \frac{2}{5}$.

$\color{red}{\text{Now, using the formulas for the unconditional mean and variance, we have}}$ 
$$\mathrm{E}\left( X \right) = \mathrm{E}\left\lbrack E\left( X\left| q \right.\  \right) \right\rbrack = \mathrm{E}\left( 100q \right) = 100\mathrm{E}\left( q \right) = 60$$
$$\mathrm{Var}\left( X \right) = \mathrm{E}\left\lbrack \mathrm{Var}\left( X\left| q \right.\  \right) \right\rbrack + \mathrm{Var}\left\lbrack E\left( X\left| q \right.\  \right) \right\rbrack = \mathrm{E}\left\lbrack 100q\left( 1 - q \right) \right\rbrack + \mathrm{Var}\left( 100q \right)$$

$$= 100\mathrm{E}\left( q \right) - 100\mathrm{E}\left( q^{2} \right) + 100^{2}\mathrm{V}\left( q \right) = 420$$.
...
Claim sizes, $X$, are uniform on for each policyholder. varies by policyholder according to an exponential distribution with mean 5. Find the unconditional distribution, mean and variance of $X$.
...
\end{matrix} \right.\ $$
$\color{red}{\text{Note that this is a case function due to the domains on which the variables are defined.}}$
...
$\color{red}{\text{An arguably simpler path to the solution is to}}$ make use of the relationship between $X$ and $Y^{P}$. If $X$ is exponentially distributed with mean 1000, then $Y^{P}$ is also exponentially distributed with the same mean, $\color{red}{\text{because of the memoryless property of the exponential distribution.}}$. Hence, $E\left( Y^{P} \right)$=1000 and
...
$\color{red}{\text{We define } Y^P \text{ as follows}$
$$Y^{P} = \left\{ \begin{matrix}
...
$\color{red}{\text{Note that we divide by S_X(4)=1-F_X(4), as this is the range where the variable} Y^P \text{ is defined.}}$
...
$\color{red}{\text{Let the losses } X\sim Exp(\theta) \text{ and the deductible for the coming year } d' = \frac{4}{3}d,\text{ the deductible of the current year.}}$
...
$$\color{red}{\frac{\theta - \theta e^{- \frac{d'}{\theta}}}{\theta}} = \frac{\theta - \theta e^{- \frac{\left( \frac{4}{3}d \right)}{\theta}}}{\theta} = 1 - e^{- \frac{\left( \frac{4}{3} d \right)}{\theta}} = 1 - \left( e^{-d /\theta} \right)^{4/3} = 1 - {0.3}^{4/3} = 0.8 .$$
...
$$\mathrm{E}\left( Y^{k} \right) = \mathrm{E}\left\lbrack \left( X \land u \right)^{k} \right\rbrack = \int_{0}^{u}x^{k}f_{X}\left( x \right)dx + \int_{u}^{\infty}{u^{k}f_{X}\left( x \right)} dx \\ \int_{0}^{u}x^{k}f_{X}\left( x \right)dx + u^{k}\left\lbrack {1 - F}_{X}\left( u \right) \right\rbrack dx.$$
...
$\color{red}{\text{Define the total amount of bills payed by the insurer as}}$
$$Y = X \land 1 = \left\{ \begin{matrix}
X & X \leq 1, \\
1 & X > 1. \\
\end{matrix} \right.\ $$
$\color{red}{So, }$
$\mathrm{E}\left( Y \right) = \mathrm{E}\left( X \land 1 \right) = \int_{0}^{1}\frac{x^{2}(4 - x)}{9}dx + \color{red}{1 *}\int_{1}^{3}\frac{x\left( 4 - x \right)}{9}dx = 0.935$.
...
$\color{red}{\text{We define the amount per loss } Y^L \text{ of both years as}}$
...
$E\left( Y_{2006}^{P} \right) = \frac{356.026}{e^{- \frac{100}{1000}}} = \color{red}{393.469}$.

$E\left( Y_{2007}^{P} \right) = \frac{361.659}{e^{- \frac{95.24}{1000}}} = \color{red}{397.797}$.

$\color{red}{\frac{E\left( Y_{2007}^{P} \right)}{E\left( Y_{2006}^{P} \right)} -1 = 0.011}$
...
$\color{red}{1.\ }$The likelihood function, $L\left( \theta \right)$, can be viewed as the probability of the observed data, written as a function of the model's
parameter $\theta$ 
...
$\color{red}{2.\ }$Taking reciprocal of negative expectation of the second derivative of $\ln L \left( \theta \right)$, we obtain an estimate of the variance of $\hat{\theta}$
...
$\color{red}{3.\ }$The 95% confidence interval for $\theta$ is given by
$$10,667 \pm 1.96\sqrt{28,446,222} = \left( 213.34,\ 21,120.66 \right).$$
$\color{red}{4.\ }$The distribution function of $X$ is $F\left( x \right) = 1 - e^{- \frac{x}{\theta}}$. Then, the maximum likelihood estimate of $g\left( \theta \right) = F\left( 9,000 \right)$ is
...
$\color{red}{1.\ }$The likelihood function, $L\left( \mu,\sigma \right)$, is the product of the pdf for each data point.
...
$\color{red}{2.\ }$ To derive the covariance matrix of the MLE we need to find the expectations of the second derivatives. Since the random variable $X$ is from a lognormal distribution with parameters $\mu$ and $\sigma$, then $\text{lnX}$ is normally distributed with mean $\mu$ and variance $\sigma^{2}$.
...
$\color{red}{3.\ }$ The 95% confidence interval for $\mu$ is given by $9.38 \pm 1.96\sqrt{0.8533} = \left( 7.57,\ 11.19 \right)$.

The 95% confidence interval for $\sigma^{2}$ is given by $5.12 \pm 1.96\sqrt{0.4267} = \left( 3.84,\ 6.40 \right)$.

$\color{red}{4.\ }$ The mean of *X* is $\exp\left( \mu + \frac{\sigma^{2}}{2} \right)$. Then, the maximum likelihood estimate of
